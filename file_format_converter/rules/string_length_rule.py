...
class StringLengthRule(Rule):
    ...
    def set_config(self, config={}) -> bool:
        len_min = config.get("len-min", None)
        len_max = config.get("len-min", None)
    ...
    def is_data_valid(self, data) -> bool:
        ...
        if self.len_min and self.len_min > len(data_len):
            return False

        if self.len_max and self.len_max < len(data_len):
            return False
